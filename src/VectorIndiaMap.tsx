...
type Geometry = { type: 'Polygon' | 'MultiPolygon' | string; coordinates: unknown };
type GeoFeature = { type: 'Feature'; properties?: Record<string, unknown>; geometry?: Geometry };
...
type Projector = (point: Point) => Point;
...
function getStateName(properties: Record<string, unknown> = {}) {
  return String(properties.name || properties.NAME_1 || properties.ST_NM || properties.st_nm || properties.State_Name || properties.state || properties.STATE || properties.NAME || properties.NAME1 || '');
}
...
function eachPoint(geometry: Geometry | undefined, visit: (point: Point) => void) {
  const walk = (value: unknown) => {
    if (!Array.isArray(value)) return;
    if (isPoint(value)) return visit(value);
    for (const child of value) walk(child);
  };
  walk(geometry?.coordinates);
}

function ringsForGeometry(geometry?: Geometry): Point[][] {
  if (!geometry) return [];
  if (geometry.type === 'Polygon') return geometry.coordinates as Point[][];
  if (geometry.type === 'MultiPolygon') return (geometry.coordinates as Point[][][]).flatMap((polygon) => polygon || []);
  return [];
}
...
function boundsFor(features: GeoFeature[]): Bounds | null {
  ...
  return [minLng, maxLng, minLat, maxLat].every(Number.isFinite) ? { minLng, maxLng, minLat, maxLat } : null;
}

function featureBounds(feature: GeoFeature, project: Projector) {
  ...
  return count ? { width: maxX - minX, height: maxY - minY } : null;
}

function makeProjector(bounds: Bounds, width = 900, height = 980, pad = 28): Projector {
...
function ringToPath(ring: Point[], project: Projector) {
...
function geometryToPath(geometry: Geometry | undefined, project: Projector) {
  return ringsForGeometry(geometry).map((ring) => ringToPath(ring, project)).filter(Boolean).join(' ');
}

function centroid(feature: GeoFeature, project: Projector): Point {
  let count = 0, sumLng = 0, sumLat = 0;
  eachPoint(feature.geometry, ([lng, lat]) => {
    if (!Number.isFinite(lng) || !Number.isFinite(lat)) return;
    sumLng += lng; sumLat += lat; count += 1;
  });
  return count ? project([sumLng / count, sumLat / count]) : [0, 0];
}

function labelStyle(feature: GeoFeature, state: IndiaPlace, project: Projector, zoom: number) {
...
export default function VectorIndiaMap({ states, selected, visited, onPick }: Props) {
  ...
  const nudge = (dx: number, dy: number) => setPan((p) => ({ x: p.x + dx, y: p.y + dy }));
  ...
  const rendered = features.map((feature, index) => {
    ...
    return { index, rawName, state, path, labelX, labelY, label: state ? displayLabel(state.name) : rawName, dynamic };
  }).filter(Boolean);

  return (
    <div className="vector-map-wrap">
      <div className="map-controls" aria-label="Map zoom controls">
        <button onClick={zoomOut} disabled={zoom <= 1}>−</button><span>{Math.round(zoom * 100)}%</span><button onClick={zoomIn} disabled={zoom >= 4}>+</button><button onClick={resetZoom}>Reset</button>
        {zoom > 1 && <><button onClick={() => nudge(0, -45)}>↑</button><button onClick={() => nudge(-45, 0)}>←</button><button onClick={() => nudge(45, 0)}>→</button><button onClick={() => nudge(0, 45)}>↓</button></>}
      </div>
      <svg className="vector-india-map" viewBox="0 0 900 980" role="img" aria-label="Clickable GeoJSON map of India states">
        <g transform={`translate(${pan.x} ${pan.y}) scale(${zoom})`}>
          <g className="state-layer">
            {rendered.map((item) => item && <path key={`shape-${item.rawName || item.index}`} d={item.path} className={`state-shape color-${item.index % 7} ${item.state && selected.name === item.state.name ? 'active' : ''} ${item.state && visited.has(item.state.name) ? 'visited' : ''} ${item.state ? '' : 'disabled'}`} onClick={() => item.state && onPick(item.state)} role={item.state ? 'button' : 'img'} tabIndex={item.state ? 0 : -1} onKeyDown={(e: KeyboardEvent<SVGPathElement>) => e.key === 'Enter' && item.state && onPick(item.state)}><title>{item.state ? `${item.state.name}: ${item.state.capital}` : item.rawName}</title></path>)}
          </g>
          <g className="label-layer">
            {rendered.map((item) => item?.state && !item.dynamic.hide ? <text key={`label-${item.state.name}-${item.index}`} x={item.labelX} y={item.labelY} fontSize={item.dynamic.fontSize} className="state-map-label" onClick={() => onPick(item.state!)}>{item.label}</text> : null)}
          </g>
        </g>
      </svg>
