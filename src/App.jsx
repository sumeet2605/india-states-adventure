...
function LearningCards({ title, subtitle, items, selected, onPick, type }) {
  return <section className="map-card"><div className="map-title">{type === 'ocean' ? '🌊' : '🌍'} {title}</div><p className="subtitle mini-subtitle">{subtitle}</p><div className="world-grid">{items.map((item) => <button key={item.name} className={`world-card ${selected.name === item.name ? 'selected' : ''}`} onClick={() => onPick(item)}><span className="world-emoji">{item.emoji}</span><strong>{item.name}</strong><small>{item.size}</small></button>)}</div></section>;
}

function WorldInfo({ item, type, speak }) {
  return <aside className="info-card"><div className="big-emoji">{item.emoji}</div><h2>{item.name}</h2><div className="fact-row"><strong>📏 Size</strong><span>{item.size}</span></div>{type === 'continent' ? <><div className="fact-row"><strong>🌐 Countries</strong><span>{item.countries}</span></div><div className="fact-row"><strong>🦁 Animals</strong><span>{item.animals}</span></div><div className="fact-row"><strong>📍 Landmark</strong><span>{item.landmark}</span></div></> : <><div className="fact-row"><strong>🗺 Touches</strong><span>{item.touches}</span></div><div className="fact-row"><strong>🐬 Animals</strong><span>{item.animals}</span></div></>}<p className="fun"><Sparkles size={18}/> {item.fact}</p><button className="speak" onClick={() => speak(item)}><Volume2 size={18}/> Read aloud</button></aside>;
}
...
function MemoryGame({ items, leftKey, rightKey, title, onWin }) {
  ...
  function pick(card) {
    ...
    if (newPicked.length === 2) {
      setTimeout(() => {
        const match = newPicked[0].pair === newPicked[1].pair;
        setCards((old) => old.map((c) => newPicked.some((p) => p.id === c.id) ? { ...c, open: match } : c));
        setPicked([]);
        if (match) onWin(5);
      }, 550);
    }
...
function Passport({ score, visitedCount }) {
  const badges = earnedBadges(score);
  return <section className="passport-card"><h2>🛂 Explorer Passport</h2><p>Visited {visitedCount} places. Stars: {score}</p><div className="badge-row">{badges.length ? badges.map((b) => <span key={b.name} className="badge">{b.emoji} {b.name}</span>) : <span className="badge locked">🔒 Earn 25 stars for first badge</span>}</div></section>;
}
...
export default function App() {
  ...
  const worldItems = [...continents, ...oceans];
  const worldQuestion = worldItems[worldQuizIndex % worldItems.length];
  const worldOptions = useMemo(() => makeOptions(worldItems, worldQuestion, (i) => i.landmark || i.size), [worldQuizIndex]);
  ...
  function speak(item) { if (!window.speechSynthesis) return; const text = item.capital ? `${item.name}. Capital ${item.capital}. Main language ${item.language}.` : `${item.name}. ${item.size}. ${item.fact}`; window.speechSynthesis.cancel(); window.speechSynthesis.speak(new SpeechSynthesisUtterance(text)); }
  ...
  function answerWorld(option) { const correct = worldQuestion.landmark || worldQuestion.size; if (option === correct) addStars(10, 'World quiz correct! +10 stars 🌎'); else setMessage(`Good try! The answer is ${correct}.`); setWorldQuizIndex((i) => i + 1); }

  return <main className="app"><section className="hero"><div><p className="eyebrow">For curious explorers age 7+</p><h1>🌎 Geography Adventure</h1><p className="subtitle">Explore India, continents, and oceans with games, stars, badges, and read-aloud learning.</p></div><div className="mascot" aria-hidden="true">🐘</div><div className="score-card"><Trophy /> <strong>{score}</strong><span>stars</span></div></section><Passport score={score} visitedCount={visited.size} />
    <nav className="tabs"><button className={section === 'india' ? 'active' : ''} onClick={() => setSection('india')}>🇮🇳 India</button><button className={section === 'continents' ? 'active' : ''} onClick={() => setSection('continents')}>🌍 Continents</button><button className={section === 'oceans' ? 'active' : ''} onClick={() => setSection('oceans')}>🌊 Oceans</button><button className={section === 'worldquiz' ? 'active' : ''} onClick={() => setSection('worldquiz')}>🎯 World Quiz</button><button className={section === 'memory' ? 'active' : ''} onClick={() => setSection('memory')}>🧠 Memory</button></nav>
    ...
    {section === 'memory' && <MemoryGame items={states} leftKey="name" rightKey="capital" title="State ↔ Capital Match" onWin={(n) => addStars(n, `Match found! +${n} stars 🧠`)} />}
  </main>;
}
