...
export function IndiaScreen(props: Props) {
  const { states, selected, visited, mastery, streak, score, mode, query, message, treasure, question, onMode, onQuery, onPick, onSpeak, onAnswer, onStartQuiz, onStartHunt, onReset } = props;
  ...
  return <>
    ...
    <div className="layout wide-map-layout">
      <section className="map-card"><div className="map-title"><MapPin /> Clickable India Map</div><LearningCoach selected={selected} visitedCount={visited.size} totalPlaces={states.length} score={score} mastery={mastery} streak={streak} onStartQuiz={onStartQuiz} onStartHunt={onStartHunt}/><VectorIndiaMap states={states} selected={selected} visited={visited} onPick={onPick}/><input className="search" placeholder="Search state, UT, capital, language..." value={query} onChange={(e) => onQuery(e.target.value)}/><div className="state-grid compact">{filtered.map((state) => <button key={state.name} className={`state-tile ${selected.name === state.name ? 'selected' : ''}`} onClick={() => onPick(state)}><span className="emoji">{state.emoji}</span><span>{state.name}</span>{visited.has(state.name) && <Star className="mini-star" size={14}/>}</button>)}</div></section>
