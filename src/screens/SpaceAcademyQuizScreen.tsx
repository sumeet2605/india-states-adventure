...
import { SpaceComicPlayer } from './SpaceComicPlayer';

type Phase = 'comic' | 'story' | 'game';

export function SpaceAcademyQuizScreen() {
  const [missionId, setMissionId] = useState(spaceMissions[0].id);
  const [phase, setPhase] = useState<Phase>('comic');
  const [fuel, setFuel] = useState(0);
  const [feedback, setFeedback] = useState('Watch the comic story first. Then unlock the game.');
  ...
  const chooseMission = (id:string) => { setMissionId(id); setPhase('comic'); setFeedback('New mission selected. Start with the comic story.'); };
  ...
  return <section className="home-adventure">
    ...
    <div className="tabs"><button className={phase==='comic'?'active':''} onClick={()=>setPhase('comic')}>🎬 Comic Video</button><button className={phase==='story'?'active':''} onClick={()=>setPhase('story')}>📖 Story Cards</button><button className={phase==='game'?'active':''} onClick={()=>setPhase('game')}>🎮 Knowledge Game</button></div>
    {phase==='comic' && <SpaceComicPlayer mission={mission} story={story} onFinish={()=>{setPhase('game');setFeedback(story.gameIntro);}} />}
